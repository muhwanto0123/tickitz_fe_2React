  ...
  const handleAvailableSeat = async () => {
    try {
      ...
      const requestSeat = await axios.post(
        `https://tickitz-be.onrender.com/gusti/movie/${slug}/seat`,
        {
  ...
  const handleCheckout = async () => {
    try {
      ...
      const requestBooking = await axios.post(
        `https://tickitz-be.onrender.com/gusti/ticket/seat`,
        {
          movieSlug: slug,
      ...
      if (requestBooking.data.data.paymentId) {
        const requestPayment = await axios.patch(
          `https://tickitz-be.onrender.com/gusti/ticket/purchase/${requestBooking.data.data.paymentId}`,
          {},
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("token")}`,
